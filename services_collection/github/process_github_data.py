# process_github_data.py
"""
This script processes the raw data from the GitHub crawler.
It prepares the data for final aggregation by:
1.  Loading the raw extracted links.
2.  Deduplicating the data based on the unique 'extracted_url'.
3.  Adds a new 'fqdn' column by extracting the domain from each URL.
...
Usage:
  - Ensure 'github_output/github_extracted_links.csv' exists.
  - Run the script: python process_github_data.py
"""
...
# --- CONFIGURATION ---
INPUT_DIR = Path("github_output")
RAW_INPUT_FILE = INPUT_DIR / "github_extracted_links.csv"
PROCESSED_OUTPUT_FILE = INPUT_DIR / "github_processed_links.csv" # Overwriting with the correct version
...
if __name__ == "__main__":
    ...
    print("--- Starting GitHub Data Processing (No Blocklisting) ---")

    # Load the raw data
    df = pd.read_csv(RAW_INPUT_FILE)
    print(f"Step 1: Loaded {len(df):,} raw link entries.")

    # Step 2: Deduplicate based on 'extracted_url', keeping the first occurrence's context.
    ...
    # Sort the data by FQDN to group similar services together
    df_sorted = df.sort_values(by='fqdn', ascending=True)

    # Ensure all original and new columns are present in the final output
    final_columns = [
        'platform', 'search_phrase', 'repository_name', 'stars', 
        'source_url', 'link_text', 'extracted_url', 'fqdn'
    ]
