# **Tailwind**
**URL:** https://v2.tailwindcss.com/docs
**Page Title:** Documentation - Tailwind CSS
--------------------

[LINK: Tailwind CSS on GitHub](https://github.com/tailwindlabs/tailwindcss)

## Getting started with Tailwind CSS

Learn Tailwind the way that best matches your learning style.

## Read the docs

Learn how to get Tailwind set up in your project.
[LINK: Start learning](/docs/installation)

## Try it in the browser

Build something with Tailwind in our online playground.

## Watch the screencasts

Learn more about Tailwind directly from the team on our channel.

## Tailwind UI

Beautiful UI components, crafted by the creators of Tailwind CSS

## What’s new in Tailwind

- Focus Ring Utilities 2.0+
[LINK: Focus Ring Utilities 2.0+](/docs/ring-width)
- Dark Mode 2.0+
[LINK: Dark Mode 2.0+](/docs/dark-mode)
- Extended Color Palette 2.0+
[LINK: Extended Color Palette 2.0+](/docs/customizing-colors#color-palette-reference)
- Extend Variants 2.0+
[LINK: Extend Variants 2.0+](/docs/configuring-variants#enabling-extra-variants)
- Extra Wide Breakpoint 2.0+
[LINK: Extra Wide Breakpoint 2.0+](/docs/breakpoints)
- Shareable Presets
[LINK: Shareable Presets](/docs/presets)
- Gradients
[LINK: Gradients](/docs/gradient-color-stops)
- Animations
[LINK: Animations](/docs/animation)
- Tailwind CSS v2.2 Jun 18, 2021 Over the last few weeks, we've been having a ton of fun dumping new and exciting features into Tailwind. Now feels like the right time to cut a release, so here it is — Tailwind CSS v2.2! We've built-in a new high-performance CLI tool, added before and after pseudo-element support, introduced new sibling selector variants, the ability to style highlighted text, and tons more.

### Tailwind CSS v2.2

Over the last few weeks, we've been having a ton of fun dumping new and exciting features into Tailwind. Now feels like the right time to cut a release, so here it is — Tailwind CSS v2.2! We've built-in a new high-performance CLI tool, added before and after pseudo-element support, introduced new sibling selector variants, the ability to style highlighted text, and tons more.
- Tailwind CSS v2.1 Apr 6, 2021 We just released Tailwind CSS v2.1 which brings the new JIT engine to core, adds first-class CSS filter support, and more!

### Tailwind CSS v2.1

We just released Tailwind CSS v2.1 which brings the new JIT engine to core, adds first-class CSS filter support, and more!
- Just-In-Time: The Next Generation of Tailwind CSS Mar 15, 2021 One of the hardest constraints we've had to deal with as we've improved Tailwind CSS over the years is the generated file size in development. With enough customizations to your config file, the generated CSS can reach 10mb or more, and there's only so much CSS that build tools and even the browser itself will comfortably tolerate.

### Just-In-Time: The Next Generation of Tailwind CSS

One of the hardest constraints we've had to deal with as we've improved Tailwind CSS over the years is the generated file size in development. With enough customizations to your config file, the generated CSS can reach 10mb or more, and there's only so much CSS that build tools and even the browser itself will comfortably tolerate.
- Welcoming James McDonald to Tailwind Labs Mar 9, 2021 Many years ago I got a message from Steve that said something like "Have I ever shared this guy's Dribbble profile with you before? Been following him forever, some of my absolute favorite work I've ever found." That person was James McDonald, and today we're totally over the moon to share that James is joining our team full-time.

### Welcoming James McDonald to Tailwind Labs

Many years ago I got a message from Steve that said something like "Have I ever shared this guy's Dribbble profile with you before? Been following him forever, some of my absolute favorite work I've ever found." That person was James McDonald, and today we're totally over the moon to share that James is joining our team full-time.
- "Tailwind CSS: From Zero to Production" on YouTube Feb 16, 2021 Today we're excited to release Tailwind CSS: From Zero to Production, a new screencast series that teaches you everything you need to know to get up and running with Tailwind CSS v2.0 from scratch.

### "Tailwind CSS: From Zero to Production" on YouTube

Today we're excited to release Tailwind CSS: From Zero to Production, a new screencast series that teaches you everything you need to know to get up and running with Tailwind CSS v2.0 from scratch.
- Welcoming David Luhr to Tailwind Labs Feb 1, 2021 We started working with David Luhr last summer on a project-by-project basis to help us develop a Figma version of Tailwind UI (almost ready!), as well as to leverage his accessibility expertise when building Tailwind UI templates, ensuring we were following best practices and delivering markup that would work for everyone, no matter what tools they use to browse the web. Today we're excited to share that David has joined the team full-time!

### Welcoming David Luhr to Tailwind Labs

We started working with David Luhr last summer on a project-by-project basis to help us develop a Figma version of Tailwind UI (almost ready!), as well as to leverage his accessibility expertise when building Tailwind UI templates, ensuring we were following best practices and delivering markup that would work for everyone, no matter what tools they use to browse the web. Today we're excited to share that David has joined the team full-time!
- Multi-line truncation with @tailwindcss/line-clamp Jan 25, 2021 Imagine you're implementing a beautiful design you or someone on your team carefully crafted in Figma. You've nailed all the different layouts at each breakpoint, perfected the whitespace and typography, and the photography you're using is really bringing the design to life.

### Multi-line truncation with @tailwindcss/line-clamp

Imagine you're implementing a beautiful design you or someone on your team carefully crafted in Figma. You've nailed all the different layouts at each breakpoint, perfected the whitespace and typography, and the photography you're using is really bringing the design to life.
- Tailwind CSS v2.0 Nov 18, 2020 Today we're finally releasing Tailwind CSS v2.0, including an all-new color palette, dark mode support, and tons more!

### Tailwind CSS v2.0

Today we're finally releasing Tailwind CSS v2.0, including an all-new color palette, dark mode support, and tons more!

## Get involved

- GitHub Discussions Connect with members of the Tailwind CSS community.
[LINK: GitHub Discussions Connect with members of the Tailwind CSS community.](https://github.com/tailwindlabs/tailwindcss/discussions)

### GitHub Discussions

Connect with members of the Tailwind CSS community.
- Discord Join our Discord group to chat with other Tailwind users.

### Discord

Join our Discord group to chat with other Tailwind users.
- Twitter Follow the Tailwind Twitter account for news and updates.

### Twitter

Follow the Tailwind Twitter account for news and updates.
- YouTube Watch screencasts and feature tutorials of Tailwind.

### YouTube

Watch screencasts and feature tutorials of Tailwind.

--------------------