# secrets
**URL:** https://rok8s-scripts.docs.fairwinds.com/secrets
**Page Title:** Managing Kubernetes Secrets Securely | Rok8s Scripts Documentation
--------------------
...
The Rok8s-scripts k8s-deploy-secrets helper script (opens new window) decrypts and deploys secrets to Kubernetes. You do not need to call this script directly, it is called by other deploy scripts.
[LINK: k8s-deploy-secrets helper script (opens new window)](https://github.com/FairwindsOps/rok8s-scripts/blob/master/bin/k8s-deploy-secrets)
...
There is an example here (opens new window) .
[LINK: here (opens new window)](https://github.com/FairwindsOps/rok8s-scripts/tree/master/examples/external-secrets-manager)

## # Encrypted Secrets With Sops

Sops and rok8s-scripts can provide a powerful solution to encrypted secret management. This enables you to keep your secrets in source control and track changes to those secrets along with the rest of your Kubernetes configuration. All of this is powered with cloud based KMS from either AWS or GCP. Using an AWS KMS (opens new window) ARN or Google KMS (opens new window) ID, Kubernetes Secret manifests are encrypted in your code repository and decrypted at deployment time.
...
An example of sops usage with rok8s-scripts can be found here (opens new window) .
[LINK: example of sops usage with rok8s-scripts can be found here (opens new window)](https://github.com/FairwindsOps/rok8s-scripts/tree/master/examples/minimal-sops-secrets)
...
- In AWS, create a KMS key (opens new window)
[LINK: create a KMS key (opens new window)](https://docs.aws.amazon.com/kms/latest/developerguide/create-keys.html)
- In GCP, create a KMS keyring and a KMS key (opens new window) - we recommend creating a single KMS keyring for sops secrets, and a KMS key per application.
[LINK: create a KMS keyring and a KMS key (opens new window)](https://cloud.google.com/kms/docs/quickstart)
We recommend using a tool like Terraform (opens new window) to manage the above, using these Terraform resources:
- aws_kms_key (opens new window)
[LINK: aws_kms_key (opens new window)](https://www.terraform.io/docs/providers/aws/r/kms_key.html)
- google_kms_key_ring (opens new window)
[LINK: google_kms_key_ring (opens new window)](https://www.terraform.io/docs/providers/google/d/google_kms_key_ring.html)
- google_kms_crypto_key (opens new window)
[LINK: google_kms_crypto_key (opens new window)](https://www.terraform.io/docs/providers/google/d/google_kms_crypto_key.html)
...
- Using IAM policyes with AWS KMS (opens new window)
[LINK: Using IAM policyes with AWS KMS (opens new window)](https://docs.aws.amazon.com/kms/latest/developerguide/iam-policies.html)
- GCP KMS predefined roles (opens new window)
[LINK: GCP KMS predefined roles (opens new window)](https://cloud.google.com/kms/docs/reference/permissions-and-roles#predefined_roles)
...
cloud provider (GCP or AWS). You can read more about sops usage on the sops usage page (opens new window) .
[LINK: the sops usage page (opens new window)](https://github.com/mozilla/sops#usage)
