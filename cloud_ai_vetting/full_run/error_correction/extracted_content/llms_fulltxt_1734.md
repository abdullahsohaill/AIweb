# llms-full.txt
**URL:** https://docs.coframe.com/llms-full.txt
**Page Title:** 
--------------------
...
1. **Maintaining an Adaptive Holdout Baseline**: A control variant is retained for to estimate the baseline performance, the holdout allocation is dynamically adjusted to optimize for maximal conversions once enough data has been collected to establish a baseline.
...
In our context, the agent aims to minimize **regret** $R(T)$, defined as:
...
1. **Continuous Evolution**: New variants are regularly introduced, keeping content fresh and aligned with traffic behavioral patterns.
...
We use Bayesian methods to estimate the credibility interval for $\hat{\mu}_i$. This provides a probabilistic range of plausible values for the mean of the underlying conversion rate, given the observed data and prior beliefs.
...
<img alt="Coframe Optimizer Chart" />
...
### (Disabled) Auto-Approve Variants
...
3. **Create a New Segment**:
   ...
   * Click on **"+ Create New"** to open the segment creation modal.
     <img alt="Creating a New Segment" />
