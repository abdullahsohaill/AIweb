...
def setup_client():
    ...
    env_path = current_dir.parent.parent / ".env"  # cloud_ai_vetting/.env
    
    if not env_path.exists():
        env_path = current_dir.parent / ".env"  # full_run/.env
        
    if not env_path.exists():
        env_path = current_dir / ".env"  # current folder
...
def generate_prompt(tool_name, url, content):
    """Refined research prompt with improved integration signals."""
    return f"""
