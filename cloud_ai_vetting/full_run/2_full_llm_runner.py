...
# --- CONFIGURATION ---
INPUT_FILE = "full_content_metadata_cleaned.csv"
OUTPUT_FILE = "full_annotated_results.csv"
...
def setup_client():
    ...
    env_path = current_dir.parent / ".env" # Check parent folder (cloud_ai_vetting/.env)
    
    if not env_path.exists():
        env_path = current_dir / ".env" # Check current folder
...
def main():
    ...
    if os.path.exists(OUTPUT_FILE):
        ...
        results_df = df.copy()
        # Initialize columns
        for col in ['llm_decision', 'llm_reasoning']:
            if col not in results_df.columns:
                results_df[col] = pd.NA
        
        # Set types to avoid pandas warnings
        results_df['llm_decision'] = results_df['llm_decision'].astype('Int64')
        results_df['llm_reasoning'] = results_df['llm_reasoning'].astype('object')
    ...
    for index, row in tqdm(results_df.iterrows(), total=len(results_df)):
        ...
        if pd.notna(row.get('llm_decision')):
            continue
        ...
        for retry in range(5):
            try:
                ...
                # Save Immediately (every row) to be safe
                results_df.to_csv(OUTPUT_FILE, index=False)
