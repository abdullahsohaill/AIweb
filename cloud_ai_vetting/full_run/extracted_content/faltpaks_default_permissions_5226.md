# Faltpaks default permissions
**URL:** https://docs.flatpak.org/en/latest/sandbox-permissions.html
**Page Title:** Sandbox Permissions - Flatpak documentation
--------------------

[LINK: View this page](https://github.com/flatpak/flatpak-docs/blob/master/docs/sandbox-permissions.rst?plain=true)
[LINK: Edit this page](https://github.com/flatpak/flatpak-docs/edit/master/docs/sandbox-permissions.rst)
...
Portals have already been mentioned in Basic concepts . They are a
...
for portals can refer to the xdg-desktop-portal API documentation for
information on how to use them.
[LINK: xdg-desktop-portal API documentation](https://flatpak.github.io/xdg-desktop-portal/)
...
(mic), sound output/playback, MIDI and ALSA sound devices in /dev/snd . This permission can be sensitive in certain situations.
--socket=pulseaudio - Access to PulseAudio. It includes sound input
(mic), sound output/playback, MIDI and ALSA sound devices in /dev/snd . This permission can be sensitive in certain situations.
...
priviledged GPG operations. The gives access to the socket in gpgconf --list-dir agent-socket . This is not commonly needed
...
only --socket=x11 and applications that do, should use --socket=fallback-x11 and --socket=wayland .
...
only allows the application to own its own namespace named by $FLATPAK_ID , subnames of it and org.mpris.MediaPlayer2.$FLATPAK_ID for MPRIS .
Furthermore, it is only allowed to talk to names matching those patterns,
the bus itself org.freedesktop.DBus and portal APIs of the form org.freedesktop.portal.* .
Access to the entire bus with --socket=system-bus or --socket=session-bus stops the filtering and using them is a security
...
D-Bus permissions needed. See Audit session or system bus traffic for more information.
Ownership
Any ownership beyond what is granted by default ie. own namespace and org.mpris.MediaPlayer2.$FLATPAK_ID is typically unnecessary
...
Using read-only access wherever possible, using the :ro option.
- Using XDG base directories to store application’s
...
the application has no home or a broader permission like host that includes home . For example, if an application hardcodes the directory ~/.foo ,
...
as tmpfs by flatpak unless overriden. A --persist=.foo bind mounts ~/.foo inside the sandbox to ~/.var/app/$FLATPAK_ID/.foo on
host thus allowing an app to persistently store data in ~/.var/app/$FLATPAK_ID/.foo which would otherwise be lost. A --persist=. will persist all directories. This does not support :create, :ro, :rw suffixes or
special values like xdg-documents . However, the directory will be
created by flatpak if it doesn’t already exist. This makes it possible to avoid configuring access to the entire home
directory, and can be useful for applications that hardcode file paths
in ~/ .
The --persist=DIR option can be used to map directories from the
user’s home directory into the sandbox filesystem. This only works if
the application has no home or a broader permission like host that includes home .
For example, if an application hardcodes the directory ~/.foo ,
without any home access and no --persist the directory will be
lost from the sandbox once exited due to the filesystem being set up
as tmpfs by flatpak unless overriden. A --persist=.foo bind mounts ~/.foo inside the sandbox to ~/.var/app/$FLATPAK_ID/.foo on
host thus allowing an app to persistently store data in ~/.var/app/$FLATPAK_ID/.foo which would otherwise be lost.
...
add a wrapper script that sets it to $XDG_RUNTIME_DIR/app/$FLATPAK_ID (tmpfs) or /var/tmp (persistent
on host).
If an application uses $TMPDIR to contain lock files you may want to
add a wrapper script that sets it to $XDG_RUNTIME_DIR/app/$FLATPAK_ID (tmpfs) or /var/tmp (persistent
on host).
...
requested with --filesystem and are not available with home, host, host-os, host-etc by default:
...
- /tmp
/tmp
- /var - Note that by default /var/{cache, config, data, tmp} inside the sandbox are the same as ~/.var/app/$FLATPAK_ID/{cache, config, data, cache/tmp} .
...
numbers. For more information about the USB IDs, you can refer to the Linux USB ID repository
...
When using --usb-list , the queries are separated by a semi-colon ; , with queries for hidden devices (i.e. those that would be
...
starting with a # are ignored. When used with flatpak override or flatpak build-finish the file is no longer needed afterwards as
...
host into the sandbox by adding --metadata=X-DConf=migrate-path=/org/example/foo/ to finish-args . The
...
like WebDAV, Google Drive, SMB etc. but backends exist for MTP/PTP, USB ,
special locations like trash:// and the local filesystem too. So the app can access, read and write data from all of these
