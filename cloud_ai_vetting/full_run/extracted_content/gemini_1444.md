# Gemini
**URL:** https://ai.google.dev/gemini-api/docs/models/gemini
**Page Title:** Gemini models  |  Gemini API  |  Google AI for Developers
--------------------

- English
[LINK: English](https://ai.google.dev/gemini-api/docs/models)
- Deutsch
[LINK: Deutsch](https://ai.google.dev/gemini-api/docs/models?hl=de)
- Español – América Latina
[LINK: Español – América Latina](https://ai.google.dev/gemini-api/docs/models?hl=es-419)
- Français
[LINK: Français](https://ai.google.dev/gemini-api/docs/models?hl=fr)
- Indonesia
[LINK: Indonesia](https://ai.google.dev/gemini-api/docs/models?hl=id)
- Italiano
[LINK: Italiano](https://ai.google.dev/gemini-api/docs/models?hl=it)
- Polski
[LINK: Polski](https://ai.google.dev/gemini-api/docs/models?hl=pl)
- Português – Brasil
[LINK: Português – Brasil](https://ai.google.dev/gemini-api/docs/models?hl=pt-br)
- Shqip
[LINK: Shqip](https://ai.google.dev/gemini-api/docs/models?hl=sq)
- Tiếng Việt
[LINK: Tiếng Việt](https://ai.google.dev/gemini-api/docs/models?hl=vi)
- Türkçe
[LINK: Türkçe](https://ai.google.dev/gemini-api/docs/models?hl=tr)
- Русский
[LINK: Русский](https://ai.google.dev/gemini-api/docs/models?hl=ru)
- עברית
[LINK: עברית](https://ai.google.dev/gemini-api/docs/models?hl=he)
- العربيّة
[LINK: العربيّة](https://ai.google.dev/gemini-api/docs/models?hl=ar)
- فارسی
[LINK: فارسی](https://ai.google.dev/gemini-api/docs/models?hl=fa)
- हिंदी
[LINK: हिंदी](https://ai.google.dev/gemini-api/docs/models?hl=hi)
- বাংলা
[LINK: বাংলা](https://ai.google.dev/gemini-api/docs/models?hl=bn)
- ภาษาไทย
[LINK: ภาษาไทย](https://ai.google.dev/gemini-api/docs/models?hl=th)
- 中文 – 简体
[LINK: 中文 – 简体](https://ai.google.dev/gemini-api/docs/models?hl=zh-cn)
- 中文 – 繁體
[LINK: 中文 – 繁體](https://ai.google.dev/gemini-api/docs/models?hl=zh-tw)
- 日本語
[LINK: 日本語](https://ai.google.dev/gemini-api/docs/models?hl=ja)
- 한국어
[LINK: 한국어](https://ai.google.dev/gemini-api/docs/models?hl=ko)
[LINK: Get API key](https://aistudio.google.com/apikey)
[LINK: Cookbook](https://github.com/google-gemini/cookbook)
[LINK: Community](https://discuss.ai.google.dev/c/gemini-api/)
[LINK: Sign in](https://ai.google.dev/_d/signin?continue=https%3A%2F%2Fai.google.dev%2Fgemini-api%2Fdocs%2Fmodels&prompt=select_account)
- On this page
- Gemini 3 Pro
- Gemini 3 Flash
- Gemini 2.5 Flash
- Gemini 2.5 Flash-Lite
- Gemini 2.5 Pro
- Gemini 2.0 Flash
- Gemini 2.0 Flash-Lite
- Model version name patterns Stable Preview Latest Experimental
- Stable
- Preview
- Latest
- Experimental
- Model deprecations
- Home
- Gemini API
[LINK: Gemini API](https://ai.google.dev/gemini-api)
- Docs
[LINK: Docs](https://ai.google.dev/gemini-api/docs)

## Gemini models

- On this page
- Gemini 3 Pro
- Gemini 3 Flash
- Gemini 2.5 Flash
- Gemini 2.5 Flash-Lite
- Gemini 2.5 Pro
- Gemini 2.0 Flash
- Gemini 2.0 Flash-Lite
- Model version name patterns Stable Preview Latest Experimental
- Stable
- Preview
- Latest
- Experimental
- Model deprecations
OUR MOST INTELLIGENT MODEL

## Gemini 3 Pro

The best model in the world for multimodal understanding, and our most powerful agentic and vibe-coding model yet, delivering richer visuals and deeper interactivity, all built on a foundation of state-of-the-art reasoning.

### Expand to learn more

Try in Google AI Studio
Inputs
Text, Image, Video, Audio, and PDF
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
65,536
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Supported
Function calling
Supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL context
Supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Preview: gemini-3-pro-preview
Inputs
Image and Text
Output
Image and Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
65,536
Output token limit
32,768
Audio generation
Not supported
Batch API
Supported
Caching
Not supported
Code execution
Not supported
File search
Not supported
Function calling
Not supported
Grounding with Google Maps
Not supported
Image generation
Supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL context
Not supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Preview: gemini-3-pro-image-preview
OUR MOST BALANCED MODEL, DESIGNED TO SCALE

## Gemini 3 Flash

Our most balanced model built for speed, scale, and frontier intelligence.

### Expand to learn more

Try in Google AI Studio
Inputs
Text, Image, Video, Audio, and PDF
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
65,536
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Supported
Function calling
Supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL context
Supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Preview: gemini-3-flash-preview
FAST AND INTELLIGENT

## Gemini 2. 5 Flash

Our best model in terms of price-performance, offering well-rounded capabilities. 2.5 Flash is best for large scale processing, low-latency, high volume tasks that require thinking, and agentic use cases.

### Expand to learn more

Try in Google AI Studio
Inputs
Text, images, video, audio
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
65,536
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Supported
Function calling
Supported
Grounding with Google Maps
Supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL context
Supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Stable: gemini-2.5-flash
Inputs
Text, images, video, audio
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
65,536
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Supported
Function calling
Supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL Context
Supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Preview: gemini-2.5-flash-preview-09-2025
Inputs
Images and text
Output
Images and text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
65,536
Output token limit
32,768
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Not Supported
File search
Not Supported
Function calling
Not supported
Grounding with Google Maps
Not supported
Image generation
Supported
Live API
Not Supported
Search grounding
Not Supported
Structured outputs
Supported
Thinking
Not Supported
URL context
Not supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Stable: gemini-2.5-flash-image
- Deprecated: gemini-2.5-flash-image-preview
Inputs
Audio, video, text
Output
Audio and text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
131,072
Output token limit
8,192
Audio generation
Supported
Batch API
Not supported
Caching
Not supported
Code execution
Not supported
File search
Not Supported
Function calling
Supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Supported
Search grounding
Supported
Structured outputs
Not supported
Thinking
Supported
URL context
Not supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Preview: gemini-2.5-flash-native-audio-preview-12-2025
- Preview: gemini-2.5-flash-native-audio-preview-09-2025
Inputs
Text
Output
Audio
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
8,192
Output token limit
16,384
Audio generation
Supported
Batch API
Supported
Caching
Not supported
Code execution
Not supported
File search
Not Supported
Function calling
Not supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Not supported
Search grounding
Not supported
Structured outputs
Not supported
Thinking
Not supported
URL context
Not supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- gemini-2.5-flash-preview-tts
ULTRA FAST

## Gemini 2. 5 Flash-Lite

Our fastest flash model optimized for cost-efficiency and high throughput.

### Expand to learn more

Try in Google AI Studio
Inputs
Text, image, video, audio, PDF
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
65,536
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Supported
Function calling
Supported
Grounding with Google Maps
Supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL context
Supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Stable: gemini-2.5-flash-lite
Inputs
Text, image, video, audio, PDF
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
65,536
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Supported
Function calling
Supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL context
Supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Preview: gemini-2.5-flash-lite-preview-09-2025
OUR ADVANCED THINKING MODEL

## Gemini 2.5 Pro

Our state-of-the-art thinking model, capable of reasoning over complex problems in code, math, and STEM, as well as analyzing large datasets, codebases, and documents using long context.

### Expand to learn more

Try in Google AI Studio
Inputs
Audio, images, video, text, and PDF
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
65,536
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Supported
Function calling
Supported
Grounding with Google Maps
Supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Supported
URL context
Supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Stable: gemini-2.5-pro
Inputs
Text
Output
Audio
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
8,192
Output token limit
16,384
Audio generation
Supported
Batch API
Supported
Caching
Not supported
Code execution
Not supported
File search
Not Supported
Function calling
Not supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Not supported
Search grounding
Not supported
Structured outputs
Not supported
Thinking
Not supported
URL context
Not supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- gemini-2.5-pro-preview-tts

## Previous Gemini models

OUR SECOND GENERATION WORKHORSE MODEL

## Gemini 2.0 Flash

Gemini 2.0 Flash is deprecated and will be shut down on March 31, 2026.

### Expand to learn more

Gemini 2.0 Flash delivers next-gen features and improved capabilities,
            including superior speed, native tool use, and a 1M token
            context window.
Try in Google AI Studio
Inputs
Audio, images, video, and text
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
8,192
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Supported
File search
Not supported
Function calling
Supported
Grounding with Google Maps
Supported
Image generation
Not supported
Live API
Not supported
Search grounding
Supported
Structured outputs
Supported
Thinking
Experimental
URL context
Not supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Latest: gemini-2.0-flash
- Stable: gemini-2.0-flash-001
- Experimental: gemini-2.0-flash-exp
OUR SECOND GENERATION FAST MODEL

## Gemini 2.0 Flash-Lite

Gemini 2.0 Flash-Lite is deprecated and will be shut down on March 31, 2026.

### Expand to learn more

A Gemini 2.0 Flash model optimized for cost efficiency and low latency.
Try in Google AI Studio
Inputs
Audio, images, video, and text
Output
Text
[LINK: [*]](/gemini-api/docs/tokens)
Input token limit
1,048,576
Output token limit
8,192
Audio generation
Not supported
Batch API
Supported
Caching
Supported
Code execution
Not supported
File search
Not supported
Function calling
Supported
Grounding with Google Maps
Not supported
Image generation
Not supported
Live API
Not supported
Search grounding
Not supported
Structured outputs
Supported
Thinking
Not Supported
URL context
Not supported
[LINK: model version patterns](/gemini-api/docs/models/gemini#model-versions)
- Latest: gemini-2.0-flash-lite
- Stable: gemini-2.0-flash-lite-001

## Model version name patterns

Gemini models are available in either stable , preview , latest , or experimental versions.

### Stable

Points to a specific stable model. Stable models usually don't change. Most
production apps should use a specific stable model.
For example: gemini-2.5-flash .

### Preview

Points to a preview model which may be used for production. Preview models will
typically have billing enabled, might come with more restrictive rate limits and
will be deprecated with at least 2 weeks notice.
For example: gemini-2.5-flash-preview-09-2025 .

### Latest

Points to the latest release for a specific model variation. This can be a
stable, preview or experimental release. This alias will get hot-swapped with
every new release of a specific model variation. A 2-week notice will
be provided through email before the version behind latest is changed.
For example: gemini-flash-latest .

### Experimental

Points to an experimental model which will typically be not be suitable for
production use and come with more restrictive rate limits. We release
experimental models to gather feedback and get our latest updates into the hands
of developers quickly.
Experimental models are not stable and availability of model endpoints is
subject to change.

## Model deprecations

For information about model deprecations, visit the Gemini deprecations page.
[LINK: Gemini deprecations](/gemini-api/docs/deprecations)
Except as otherwise noted, the content of this page is licensed under the Creative Commons Attribution 4.0 License , and code samples are licensed under the Apache 2.0 License . For details, see the Google Developers Site Policies . Java is a registered trademark of Oracle and/or its affiliates.
[LINK: Google Developers Site Policies](https://developers.google.com/site-policies)
Last updated 2026-01-22 UTC.

--------------------